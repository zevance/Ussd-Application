...
def call_advisor_flow(session, msisdn, msg):
    ...
    # If no phone number entered yet, prompt for it
    if 'advisor_phone_prompted' not in session:
        session['advisor_phone_prompted'] = True
        session.modified = True
        return generate_response_xml("Lowetsani nambala ya mlangizi (e.g. 0888123456):", 2)
    # Validate phone number
    phone = msg.strip()
    if phone.startswith('+265') and len(phone) == 13 and phone[1:].isdigit():
        valid = True
        ...
        valid = False
    if not valid:
        return generate_response_xml("Nambala ya mlangizi yolakwika. Lowetsani nambala yoyenera (e.g. 0888123456):", 2)
    # Success: return the number only, with type=3 to trigger call if supported
    session.pop('advisor_phone_prompted', None)
    session.modified = True
    return generate_response_xml(phone, 3)
...
# def fetch_approved_policies(msisdn, session):
#     """Fetches approved policies that can be paid"""
#     try:
#         url = f"{API_BASE_URL}/proposals/all?username={msisdn}&status=1"  # Status 1 = approved
#         headers = {
#             "Authorization": f"Bearer {API_KEY}",
#             "Accept": "application/json"
#         }
...
#             policies = response.json().get('data', [])
#             if not policies:
#                 return generate_response_xml("Palibe ma polise ololedwa omwe alipo.", 2)
                
#             session['approved_policies'] = policies
#             session['current_policy_page'] = 1
#             session.modified = True
...
#             response_msg += "98. Lipira Polise\n" if len(policies) > 0 else ""
#             response_msg += "0. Bwelerani\n00. Koyambirira"
            
#             return generate_response_xml(response_msg, 2)
            
#         return generate_response_xml("Zolakwika pakutenga ma polise. Yesaninso.", 2)
        
#     except Exception as e:
#         print(f"Error fetching approved policies: {str(e)}")
            ...
            if response.status_code == 200:
                try:
                    ...
                    if not isinstance(payment_response, dict):
                        # Unexpected response, treat as error or success based on your API docs
                        if payment_response == 0:
                            return generate_response_xml("Malipiro anu atsimikiziridwa! Zikomo.", 3)
                        else:
