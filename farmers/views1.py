        ...
        try:
            ...
            if api_response.status_code == 200:
                ...
                for idx, district in enumerate(districts):
                    ...
                    normalized_districts.append({'id': district_id, 'name': district_name})
                
                session.update({
                    'districts': normalized_districts,
                    'region': region,
                    'current_district_page': 1,
                    'total_district_pages': (len(normalized_districts) + 9) // 10,
                    'selected_district': None
                })
                session.modified = True
                print(f"Normalized {len(normalized_districts)} districts for region: {region}")
                return generate_district_list_response(session)
            ...
            if response.status_code == 200:
                ...
                for idx, epa in enumerate(epas):
                    if isinstance(epa, dict):
                        ...
                        epa_id = str(idx + 1)
                        epa_name = str(epa)
                    normalized_epas.append({'id': epa_id, 'name': epa_name})
                
                session.update({
                    'epas': normalized_epas,
                    'district': district,
                    'current_page': 1,
                    'total_pages': (len(normalized_epas) + 9) // 10,
                    'selected_epa': None
                })
                session.modified = True
                print(f"Normalized {len(normalized_epas)} EPAs for district: {district}")
                return generate_epa_list_response(session)
...
def fetch_policies_and_respond(msisdn):
    """Fetches user policies from API."""
    ...
    try:
        ...
        if api_response.status_code == 200:
            ...
            if not policies:
                return generate_response_xml("Palibe ma polise omwe apezeka.\n0. Bwelerani\n00. Koyambilira", 2)
            response_message = "Ma polise omwe muli nawo:\n"
            for idx, policy in enumerate(policies, 1):
                response_message += f"{idx}. {policy['policy_name']} - {policy['status']}\n"
            response_message += "0. Bwelerani\n00. Koyambilira"
            return generate_response_xml(response_message, 2)
        print(f"Error fetching policies: status={api_response.status_code}, response={api_response.text}")
